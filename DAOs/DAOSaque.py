...
class DAOSaque(DAO):
    def __init__(self, arquivo: str):
        ...
        self._DAOSaque__conteudo = []
        if os.path.exists(self.__arquivo):
            try:
                self._DAOSaque__conteudo = self.__load()
    ...
    def conteudo(self, valor: list):
        self._DAOSaque__conteudo = valor

    def create(self, id: int, obs: str, valor: float, data: str) -> bool: 
        tamanho = len(self.conteudo)
        novo_saque = Saque(ident=id, obs=obs, valor=valor, data=data)
        self.conteudo.append(novo_saque)
        self.__dump()
        self.__load()
